/**
 * ╔══════════════════════════════════════════════════════════════╗
 * ║           AVANT — COMPLETE FREE API CONFIGURATION           ║
 * ║   Every API below is 100% FREE — no credit card needed      ║
 * ║   Sign-up links included for each key                       ║
 * ╚══════════════════════════════════════════════════════════════╝
 */

// ════════════════════════════════════════════════════════════════
// ⚡ AI BRAINS — Multi-LLM Cascade (ALL FREE)
// ════════════════════════════════════════════════════════════════

// 1. GROQ — Llama 4 Scout + Llama 3.3 70B — FASTEST AI on Earth
//    Free: 14,400 req/day | 2000+ tokens/sec | No credit card
//    Sign up: https://console.groq.com
export const GROQ_API_KEY = 'YOUR_GROQ_API_KEY';

// 2. GEMINI — Google Gemini 2.5 Flash — Smartest free model
//    Free: 1,500 req/day | 1M tokens/day | No credit card
//    Sign up: https://aistudio.google.com/apikey
export const GEMINI_API_KEY = 'YOUR_GEMINI_API_KEY';

// 3. DEEPSEEK — DeepSeek V3 — 5M FREE tokens on signup
//    Free: 60 RPM | 5M tokens free | No credit card
//    Sign up: https://platform.deepseek.com
export const DEEPSEEK_API_KEY = 'YOUR_DEEPSEEK_API_KEY';

// 4. CEREBRAS — Llama 3.3 70B — 1M FREE tokens/day
//    Free: 1M tokens/day | 2200+ tokens/sec | No credit card
//    Sign up: https://cloud.cerebras.ai
export const CEREBRAS_API_KEY = 'YOUR_CEREBRAS_API_KEY';

// 5. TOGETHER AI — 100+ open-source models — $5 free credits
//    Free: $5 signup credits | 100+ models | No credit card
//    Sign up: https://api.together.ai
export const TOGETHER_API_KEY = 'YOUR_TOGETHER_API_KEY';

// 6. MISTRAL — Mistral Large — 1B free tokens/month
//    Free: generous free tier | EU-based | No credit card
//    Sign up: https://console.mistral.ai
export const MISTRAL_API_KEY = 'YOUR_MISTRAL_API_KEY';

// 7. OPENAI — GPT-4o (you already have this)
//    Sign up: https://platform.openai.com
export const OPENAI_API_KEY = 'YOUR_OPENAI_API_KEY';

// ════════════════════════════════════════════════════════════════
// 🔍 SEARCH & MAPS — Live Internet Data
// ════════════════════════════════════════════════════════════════

// 8. SERPAPI — Google Search/Maps/News/Flights/Shopping/+40 more
//    Free: 100 searches/month | No credit card to start
//    Sign up: https://serpapi.com
export const SERPAPI_KEY = 'YOUR_SERPAPI_KEY';

// 9. SERPER.DEV — Google Search API — 2,500 FREE searches
//    Free: 2,500 searches free (lifetime) | No credit card
//    Sign up: https://serper.dev
export const SERPER_API_KEY = 'YOUR_SERPER_API_KEY';

// 10. GOOGLE MAPS DEMO KEY — Maps/Directions/Places
//     Free: Demo key no credit card | Prototype without billing
//     Get key: https://mapsplatform.google.com/maps-demo-key/
export const GOOGLE_MAPS_KEY = 'YOUR_GOOGLE_MAPS_DEMO_KEY';

// ════════════════════════════════════════════════════════════════
// 🌤️ WEATHER — NO KEY NEEDED (but key = more data)
// ════════════════════════════════════════════════════════════════

// 11. OPEN-METEO — ZERO key needed — 10,000 calls/day FREE
//     No signup. No key. Best free weather API on earth.
//     Docs: https://open-meteo.com/en/docs
//     Usage: just call the URL directly (see weatherModule)
export const OPEN_METEO_BASE = 'https://api.open-meteo.com/v1/forecast';

// 12. OPENWEATHER — More data (UV, air quality, alerts)
//     Free: 1,000 calls/day | 60 calls/min | No credit card
//     Sign up: https://openweathermap.org/api
export const OPENWEATHER_KEY = 'YOUR_OPENWEATHER_KEY';

// ════════════════════════════════════════════════════════════════
// 📰 NEWS — World News from 120,000+ Sources
// ════════════════════════════════════════════════════════════════

// 13. CURRENTS API — 600 req/day FREE — 120K+ sources worldwide
//     Free: 600 req/day | No credit card
//     Sign up: https://currentsapi.services/en/register
export const CURRENTS_API_KEY = 'YOUR_CURRENTS_API_KEY';

// 14. GNEWS — 100 req/day FREE — Google News aggregator
//     Free: 100 req/day | No credit card
//     Sign up: https://gnews.io/register
export const GNEWS_API_KEY = 'YOUR_GNEWS_API_KEY';

// 15. NEWSAPI — 100 req/day FREE (developer plan)
//     Free: 100 req/day | Headlines from 80,000 sources
//     Sign up: https://newsapi.org/register
export const NEWSAPI_KEY = 'YOUR_NEWSAPI_KEY';

// ════════════════════════════════════════════════════════════════
// 🚀 NASA & SPACE — 100% FREE
// ════════════════════════════════════════════════════════════════

// 16. NASA API — APOD, Mars Rover, EPIC Earth, Asteroids, more
//     Free: DEMO_KEY works. Register for 1,000 req/hour.
//     Sign up: https://api.nasa.gov
export const NASA_API_KEY = 'DEMO_KEY'; // Replace after signup

// NASA Image Search (no key needed):
export const NASA_IMAGES_BASE = 'https://images-api.nasa.gov/search';

// ════════════════════════════════════════════════════════════════
// 🌍 GEOLOCATION & COUNTRIES — NO KEY NEEDED
// ════════════════════════════════════════════════════════════════

// 17. REST COUNTRIES — No key — Country data, flags, currencies
export const REST_COUNTRIES_BASE = 'https://restcountries.com/v3.1';

// 18. IPAPI — No key — IP to city/country/lat-lon
export const IPAPI_BASE = 'https://ipapi.co/json/';

// 19. NOMINATIM (OpenStreetMap) — No key — Free geocoding
export const NOMINATIM_BASE = 'https://nominatim.openstreetmap.org';

// ════════════════════════════════════════════════════════════════
// 💰 FINANCE & CRYPTO — Free Tiers
// ════════════════════════════════════════════════════════════════

// 20. COINGECKO — Crypto prices — No key for basic endpoints
export const COINGECKO_BASE = 'https://api.coingecko.com/api/v3';

// 21. EXCHANGERATE-API — Currency conversion
//     Free: 1,500 req/month | No credit card
//     Sign up: https://app.exchangerate-api.com/sign-up
export const EXCHANGERATE_KEY = 'YOUR_EXCHANGERATE_KEY';

// ════════════════════════════════════════════════════════════════
// 🎬 MEDIA & ENTERTAINMENT — Free Tiers
// ════════════════════════════════════════════════════════════════

// 22. TMDB — Movies, TV shows, posters, ratings
//     Free: Unlimited | No credit card
//     Sign up: https://www.themoviedb.org/signup
export const TMDB_API_KEY = 'YOUR_TMDB_API_KEY';

// 23. UNSPLASH — High-quality images search
//     Free: 50 req/hour | No credit card
//     Sign up: https://unsplash.com/developers
export const UNSPLASH_KEY = 'YOUR_UNSPLASH_KEY';

// ════════════════════════════════════════════════════════════════
// 📚 KNOWLEDGE — NO KEY NEEDED
// ════════════════════════════════════════════════════════════════

// 24. WIKIPEDIA — No key — Full encyclopedia search
export const WIKIPEDIA_BASE = 'https://en.wikipedia.org/api/rest_v1';

// 25. DICTIONARY — No key — Word definitions, synonyms
export const DICTIONARY_BASE = 'https://api.dictionaryapi.dev/api/v2/entries/en';

// 26. OPEN LIBRARY — No key — Books, authors, covers
export const OPEN_LIBRARY_BASE = 'https://openlibrary.org';

// 27. QUOTABLE — No key — Inspirational quotes
export const QUOTABLE_BASE = 'https://api.quotable.io';

// ════════════════════════════════════════════════════════════════
// ✈️ FLIGHTS & TRAVEL — Free Tiers
// ════════════════════════════════════════════════════════════════

// 28. AVIATIONSTACK — Live flight tracking
//     Free: 100 req/month | No credit card
//     Sign up: https://aviationstack.com/signup/free
export const AVIATIONSTACK_KEY = 'YOUR_AVIATIONSTACK_KEY';

// ════════════════════════════════════════════════════════════════
// 🎵 AUDIO — Voice Recognition
// ════════════════════════════════════════════════════════════════

// 29. GROQ WHISPER — FREE voice-to-text via Groq
//     Uses same GROQ_API_KEY above — 7,200 audio min/day free
//     Fastest Whisper inference on earth

// ════════════════════════════════════════════════════════════════
// ⚙️ APP SETTINGS
// ════════════════════════════════════════════════════════════════

export const OWNER_NAME     = 'Michael';
export const WAKE_WORD      = 'avant';
export const HOME_ADDRESS   = 'Your Home Address Here';
export const TIMEZONE       = 'America/Los_Angeles';

// Voice
export const VOICE_PITCH    = 1.1;   // Feminine, natural
export const VOICE_RATE     = 0.95;  // Slightly measured
export const VOICE_LANG     = 'en-US';

// LLM routing priority (fastest → smartest fallback)
export const LLM_PRIORITY   = ['groq', 'cerebras', 'deepseek', 'gemini', 'mistral', 'together', 'openai'];
